...
def blog_detail(blog_model_id, username, blog_model_category):
    ...
    if request.method == 'GET':
        if current_user.id != blog_model.blog_user_id:
            ...
            db.session.commit()
        rating = db.session.query(func.avg(BlogComment.blog_rating)).filter(BlogComment.blog_id == int(blog_model_id)).first()[0]
        ...
        old_comment = BlogComment.query.filter(BlogComment.blog_id == blog_model_id).filter(BlogComment.comment_user_id == current_user.id).first()
